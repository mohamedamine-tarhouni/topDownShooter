import os
from model.player import PLAYER
import model.database as db
#les couleurs
WHITE = (255,255,255)
BLACK = (0,0,0)
RED = (255,0,0)
GREEN = (0,255,0)
BLUE = (0,0,255)
#la taille de l'écran
WIDTH=1500
HEIGHT=720
#le fichier des images
game_folder=os.path.dirname(__file__)
img_folder=game_folder
# img_folder=os.path.join(game_folder,"img")
player1Data=db.getPlayerData(1)
# print(player1Data)
player2Data=db.getPlayerData(2)
if player1Data:
    player1 = PLAYER(WIDTH-50, HEIGHT-50,"Man 1.png",player1Data[0][0],score=0
    ,HP=player1Data[0][2],Dmg=player1Data[0][3],atkSpeed=player1Data[0][4],lives=player1Data[0][1])
else:
    player1 = PLAYER(WIDTH-50, HEIGHT-50,"Man 1.png","Midouch",atkSpeed=50) # joueur1
if player2Data:
    player2 = PLAYER(WIDTH-(player1.rect.x), HEIGHT-(player1.rect.y),"Man 1.png",player2Data[0][0],score=0
    ,HP=player2Data[0][2],Dmg=player2Data[0][3],atkSpeed=player2Data[0][4],lives=player2Data[0][1])
else:
    player2 = PLAYER(WIDTH-(player1.rect.x), HEIGHT-(player1.rect.y),"Man 1.png","CDM") # joueur2
    
def InitSettings():
    if player1Data:
        player1 = PLAYER(WIDTH-50, HEIGHT-50,"Man 1.png",player1Data[0][0],score=0
        ,HP=player1Data[0][2],Dmg=player1Data[0][3],atkSpeed=player1Data[0][4],lives=player1Data[0][1])
    else:
        player1 = PLAYER(WIDTH-50, HEIGHT-50,"Man 1.png","Midouch",atkSpeed=50) # joueur1
    if player2Data:
        player2 = PLAYER(WIDTH-(player1.rect.x), HEIGHT-(player1.rect.y),"Man 1.png",player2Data[0][0],score=0
        ,HP=player2Data[0][2],Dmg=player2Data[0][3],atkSpeed=player2Data[0][4],lives=player2Data[0][1])
    else:
        player2 = PLAYER(WIDTH-(player1.rect.x), HEIGHT-(player1.rect.y),"Man 1.png","CDM") # joueur2
    return player1,player2